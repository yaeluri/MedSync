...
export const AiSummaryCard: React.FC<AiSummaryCardProps> = ({ overview, onStartVisit }) => (
  <Paper elevation={0} sx={{ borderRadius: 3, border: '1px solid #e9ecef', borderRight: '4px solid #3b5bdb', p: 2.5 }}>
    <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between', mb: 1.5, gap: 2 }}>
      ...
      <Button variant="contained" startIcon={<PlayArrowIcon />} onClick={onStartVisit} sx={{ borderRadius: 2, fontWeight: 600, fontSize: 13, flexShrink: 0 }}>
        התחל ביקור
      </Button>
    </Stack>
    <Typography sx={{ fontSize: 14, color: '#495057', lineHeight: 1.7 }}>{overview}</Typography>
  </Paper>
);
