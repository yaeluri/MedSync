...
import { getPatientById, Patient, PatientDocument, Encounter } from '../api/patients';
import { uploadDocument } from '../api/documents';

export type ToastState = { severity: 'success' | 'error'; message: string } | null;
...
function initialsFromName(name: string): string {
  return (
    name.split(/\s+/).filter(Boolean).slice(0, 2).map(s => s.charAt(0).toUpperCase()).join('') || '?'
  );
}
...
export function usePatientDashboard() {
  ...
  const refreshDocuments = useCallback(async () => {
    ...
    try {
      ...
      setDocuments(p.documents || []);
    } catch { /* ignore — list will refresh on next successful load */ }
  }, [patientId]);
